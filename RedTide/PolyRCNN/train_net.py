...
class Trainer(DefaultTrainer):
    ...
    def build_evaluator(cls, cfg, dataset_name, output_folder=None):
        """
        构建评估器，也就是在模型训练完成之后，对模型性能进行评估的工具
        """
...
def register_my_dataset(dataset_name="aicrowd_train",
    ...
    """通过detectron2框架注册COCO格式的数据集.

       usage::
       from detectron2.data import DatasetCatalog, MetadataCatalog
       from detectron2.data.datasets.coco import load_coco_json

    :param TRAIN_JSON: 标注json文件
    :param TRAIN_PATH: 图像文件夹路径
    """
    # CLASS_NAMES = []
    DatasetCatalog.register(dataset_name, lambda: load_coco_json(TRAIN_JSON, TRAIN_PATH, dataset_name, ["cor_cls_poly"]))
    # 原始的coco格式的文件里边键没有"cor_cls_poly"，所以这里要加上
...
def main(args):
    ...
    register_my_dataset(dataset_name="aicrowd_train",
                        TRAIN_JSON="./data/train/annotation_preprocessed.json",
                        TRAIN_PATH="./data/train/images")
    if args.eval_only:
