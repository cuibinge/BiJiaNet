...
class PolyRCNN(nn.Module):
    ...
    def forward(self, batched_inputs, do_postprocess=True):
        ...
        # print(features[0].shape)  # torch.Size([1, 256, 64, 64])
        # print(features[1].shape)  # torch.Size([1, 256, 32, 32])
        # print(features[2].shape)  # torch.Size([1, 256, 16, 16])
        # print(features[3].shape)  # torch.Size([1, 256, 8, 8])

        # 准备提议框
        proposal_boxes = self.init_proposal_boxes.weight.clone()
        # print(proposal_boxes.shape)  # torch.Size([100, 4])
        proposal_boxes = box_cxcywh_to_xyxy(proposal_boxes)
        # print(proposal_boxes.shape)  # torch.Size([100, 4])
        proposal_boxes = proposal_boxes[None] * images_whwh[:, None, :]
        # print(proposal_boxes.shape)  # torch.Size([1, 100, 4])
        bs, num_proposals, _ = proposal_boxes.shape

        # 准备提议多边形
        proposal_polygons = self.init_proposal_polygons.weight.clone()
        # print(proposal_polygons.shape)  # torch.Size([100, 192])
        proposal_polygons = proposal_polygons[None] * images_whwh.repeat(1, int(self.num_corners / 2))[:, None, :]
        # print(proposal_polygons.shape)  # torch.Size([1, 100, 192])
        ...
        # print("outputs_class:", outputs_class.shape)  # torch.Size([6, 1, 100, 1]) 6表示迭代了6次 100个提议框是目标的概率
        # print("outputs_coord:", outputs_coord.shape)  # torch.Size([6, 1, 100, 4]) 100个提议框的坐标系
        # print("outputs_corners:", outputs_corners.shape)  # torch.Size([6, 1, 100, 96]) 100个提议框中每个提议框中是角点的概率
        # print("outputs_polygons:", outputs_polygons.shape)  # torch.Size([6, 1, 100, 192]) 100个提议框中每个提议框中的点的坐标
    ...
    def prepare_targets(self, targets, names):
        ...
        for i in range(len(targets)):
            ...
            target["height"] = h
            target["weight"] = w
            target["labels"] = gt_classes.to(self.device).long()
    ...
    def inference(self, box_cls, box_pred, image_sizes, polygon_pred, corner_cls):
        ...
        if self.use_focal:
            ...
            # print(scores.shape)  # torch.Size([1, 100, 1])
            ...
            # print(corner_scores)  # torch.Size([1, 100, 96])
