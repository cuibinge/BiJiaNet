...
# from albumentations import Compose, RandomRotate90, HorizontalFlip, VerticalFlip, OneOf, HueSaturationValue, \
...
#
#     # 获取所有测试图片的文件路径
#     img_ids = glob(os.path.join('inputs', config['dataset'], 'test\\image', '*' + config['img_ext']))
#     # 只提取文件名（去除扩展名）
#     img_ids = [os.path.splitext(os.path.basename(p))[0] for p in img_ids]
#
#     # 直接使用所有图片作为验证集
...
import archs
from dataset import Dataset
from metrics import iou_score, accuracy, recall, f1_score
from utils import AverageMeter
...
def main():
    ...
    with open('models/%s/config.yml' % args.name, 'r') as f:
        config = yaml.load(f, Loader=yaml.FullLoader)
        config['dataset'] = '4096'  # Ensure this matches your dataset configuration
    ...
    img_ids = glob(os.path.join('inputs', config['dataset'], 'test\\image', '*' + config['img_ext']))
    img_ids = [os.path.splitext(os.path.basename(p))[0] for p in img_ids]
    val_img_ids = img_ids  # Using all images for validation
    ...
    with torch.no_grad():
        ...
        for input, target, meta in val_loader:
            ...
            λ = meta['wavelengths'].float().cuda(non_blocking=True)  # Get wavelengths from metadata

            # Compute output
            if config['deep_supervision']:
                output = model(input, λ)[-1]  # Take the last output if deep supervision
            else:
            ...
            for i in range(len(output)):
                for c in range(config['num_classes']):
                    cv2.imwrite(
                        os.path.join('outputs', config['name'], str(c), meta['img_id'][i] + '.png'),
                        (output[i, c] * 255).astype('uint8')
                    )
    ...
    print('F1-Score: %.4f' % avg_meters['f1'].avg)
