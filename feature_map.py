    ...
    def _plot_feature(feat, title, position, max_channels):
        ...
        if len(feat.shape) == 3:
            ...
            if combined.shape[2] == 2:
                ...
                pseudo_color[:, :, 1] = combined[:, :, 1]  # 第二个通道映射到绿色
                pseudo_color[:, :, 2] = 0  # 蓝色通道设置为零
                im = plt.imshow(pseudo_color)
            else:
                im = plt.imshow(combined)
        else:
            im = plt.imshow(feat, cmap='viridis')

        # 添加颜色条
        plt.colorbar(im, fraction=0.046, pad=0.04)  # 调整颜色条的大小和间距
        plt.title(f"{title}\n{feat.shape}")
        plt.axis('off')
...
if __name__ == "__main__":
    ...
    # 2. 创建可视化工具
    img_dir = r"F:\develop\py_file\unet++\inputs\256\test\image"  # 替换为TIFF图像目录
    visualizer = FeatureVisualizer(model, img_dir=img_dir, device=device)

    # 3. 选择要分析的图像ID（不带扩展名）
    img_id = "0_24_35"  # 替换为你的图像ID

    # 4. 获取并可视化特征图
    features = visualizer.get_feature_maps(img_id)
    visualizer.visualize(features, save_dir="./feature_maps/0_24_35")