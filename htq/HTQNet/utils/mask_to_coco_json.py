...
def mask_to_coco_json(image_dir, mask_dir, output_json_path):
    """
    将四通道TIFF图片和单通道TIFF掩码转换为COCO格式的JSON标注文件。

    Args:
        image_dir (str): 存放训练图片的文件夹路径（四通道TIFF格式）。
        mask_dir (str): 存放对应二值掩码的文件夹路径（单通道TIFF格式）。
        output_json_path (str): 输出的JSON文件路径。
    """

    # 初始化COCO数据结构
    coco_data = {
        "info": {"description": "Converted from TIFF masks"},
        "licenses": [],
        "images": [],
        "annotations": [],
