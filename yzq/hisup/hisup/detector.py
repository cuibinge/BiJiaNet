...
def sigmoid_l1_loss(logits, targets, offset=0.0, mask=None):
    logp = torch.sigmoid(logits) + offset
    if logp.shape != targets.shape:
        raise ValueError(f"Shape mismatch: logp.shape={logp.shape}, targets.shape={targets.shape}")
    loss = torch.abs(logp - targets)
    if mask is not None:
        t = ((mask == 1) | (mask == 2)).float()
        w = t.mean(3, True).mean(2, True)
...
class ECA(nn.Module):
    def __init__(self, channel, gamma=2, b=1):
        ...
        C = channel
#         t = int(abs((log(C, 2) + b) / gamma))
        t = int(abs((np.log2(C) + b) / gamma))  # 使用 np.log2() 直接计算以 2 为底的对数
        k = t if t % 2 else t + 1
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.conv = nn.Conv1d(1, 1, kernel_size=k, padding=int(k / 2), bias=False)
        self.sigmoid = nn.Sigmoid()
...
class BuildingDetector(nn.Module):
    def __init__(self, cfg, test=False):
        ...
        self.mask_predictor = self._make_predictor(dim_in, 3)
        self.jloc_predictor = self._make_predictor(dim_in, 3)
        self.afm_predictor = self._make_predictor(dim_in, 2)

        self.refuse_conv = self._make_conv(3, dim_in // 2, dim_in)
        self.final_conv = self._make_conv(dim_in * 2, dim_in, 2)
    ...
    def forward_train(self, images, annotations=None):
        ...
        logger.info(f"Targets 'joff' shape: {targets['joff'].shape}")
        logger.info(f"Targets 'joff' unique values: {torch.unique(targets['joff'])}")
        logger.info(f"Targets 'jloc' shape: {targets['jloc'].shape}")
        logger.info(f"Targets 'jloc' unique values: {torch.unique(targets['jloc'])}")
        ...
        afm_conv = self.refuse_conv(afm_pred)
        remask_pred = self.final_conv(torch.cat((features, afm_conv), dim=1))
        print("logp shape:", logp.shape)  # 应为 [batch_size, 2, height, width]
        print("targets['joff'] shape:", targets['joff'].shape)  # 应为 [batch_size, 2, height, width]
        print("afm_pred shape:", afm_pred.shape)  # 应为 [batch_size, 2, height, width]
        print("targets['afmap'] shape:", targets['afmap'].shape)  # 应为 [batch_size, 2, height, width]

        if targets is not None:
#             loss_dict['loss_jloc'] += self.junc_loss(jloc_pred, targets['jloc'].squeeze(dim=1))
            loss_dict['loss_jloc'] += self.junc_loss(jloc_pred, targets['jloc'].squeeze(dim=1).long())

#             loss_dict['loss_joff'] += sigmoid_l1_loss(outputs[:, :self.cfg.MODEL.NUM_CLASSES], targets['joff'], -0.5, targets['jloc'])
            loss_dict['loss_joff'] += sigmoid_l1_loss(outputs[:, :2], targets['joff'], -0.5, targets['jloc'])
