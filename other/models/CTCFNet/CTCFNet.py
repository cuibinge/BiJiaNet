...
class CTCFNet(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dims=[64, 128, 256, 512],
        ...
        self.conv128to256 = nn.ConvTranspose2d(in_channels=4, out_channels=4, kernel_size=4, stride=2, padding=1, bias=False)
    ...
    def forward(self, x):
        ...
        x = x + nn.Parameter(torch.zeros_like(x))
        # x = x + self.pos_embed1
        x = self.pos_drop1(x)
        ...
        if self.training:
            return F.interpolate(output, size=128, mode='bilinear', align_corners=True), \
                   self.detail_head(m_s2)
        else:
            return F.interpolate(output, size=256, mode='bilinear', align_corners=True)


if __name__ == '__main__':
    x = torch.rand((2, 4, 128, 128))
    model = CTCFNet(img_size=256, in_chans=4, class_dim=10,
    ...
    #     # 将张量转换为NumPy数组（如果需要的话，先调用.detach()来分离梯度）
    #     np_array = output.detach().numpy()
    #
    #     # 打印NumPy数组的形状
    #     print(f"Output {i + 1} shape: {np_array.shape}")