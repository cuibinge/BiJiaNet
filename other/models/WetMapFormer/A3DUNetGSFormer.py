...
num_patch_x = input_shape[1] // patch_size[0]
num_patch_y = input_shape[2] // patch_size[1]
...
def get_ST_model(output_units = 10):
    input_shape1 = 4, 128, 128
    input1 = Input(shape=input_shape1)
    input1_ = tf.transpose(input1, perm=[0, 2, 3, 1])
    ############################ Feature extractor
    ...
    batch_size = tf.shape(x)[0]  # 获取 batch_size 的动态值
    # new_shape = [batch_size, 36, 36, 32]
    x = tf.reshape(x, (batch_size, 32, 32, 32))
    # x = layers.GlobalAveragePooling1D()(x)
    x = layers.Dense(50, activation="relu")(x)
    x = upsample_layer = UpSampling2D(size=(4, 4))(x)
    #############################################
