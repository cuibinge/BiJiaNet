...
def activation_by_name(inputs, activation="relu", name=None):
    ...
    if activation_lower == "leaky_relu":
        ...
        # print(f"{shared_axes = }")
        return layers.PReLU(shared_axes=shared_axes, alpha_initializer=initializers.Constant(0.25), name=layer_name)(
            inputs)
        ...
        return ZeroInitGain(use_bias=True, weight_init_value=1.0, name=layer_name)(
            functional.pow(functional.relu(inputs), 2))
    else:
...
class EvoNormalization(layers.Layer):
    def __init__(self, nonlinearity=True, num_groups=-1, zero_gamma=False, momentum=0.99, epsilon=0.001,
                 data_format="auto", **kwargs):
        ...
        self.is_channels_first = (
            True if data_format == "channels_first" or (
                        data_format == "auto" and backend.image_data_format() == "channels_first") else False
        )
    ...
    def build(self, input_shape):
        ...
        self.gamma = self.add_weight(name="gamma", shape=param_shape,
                                     initializer="zeros" if self.zero_gamma else "ones", trainable=True)
        ...
        if self.num_groups > 0:  # EVONORM_S0
            ...
            if self.is_channels_first:
                ...
                self.group_reduction_axes = list(range(1, len(self.group_shape) - 2)) + [
                    len(self.group_shape) - 1]  # [1, 2, 4]
                self.group_axes = -1
                self.var_shape = [-1, *param_shape[1:]]
...
def batchnorm_with_activation(
        inputs, activation=None, zero_gamma=False, epsilon=1e-5, momentum=0.9, axis="auto", act_first=False,
        use_evo_norm=False, evo_norm_group_size=-1, name=None
):
    ...
    if use_evo_norm:
        ...
        return EvoNormalization(nonlinearity, num_groups=num_groups, zero_gamma=zero_gamma, epsilon=epsilon,
                                momentum=momentum, name=name + "evo_norm")(inputs)
...
def layer_norm(inputs, zero_gamma=False, epsilon=LAYER_NORM_EPSILON, center=True, axis="auto", name=None):
    ...
    return layers.LayerNormalization(axis=norm_axis, epsilon=epsilon, gamma_initializer=gamma_init, center=center,
                                     name=name and name + "ln")(inputs)
...
def conv2d_no_bias(inputs, filters, kernel_size=1, strides=1, padding="valid", use_bias=False, groups=1,
                   use_torch_padding=True, name=None, **kwargs):
...
def depthwise_conv2d_no_bias(inputs, kernel_size, strides=1, padding="valid", use_bias=False, use_torch_padding=True,
                             name=None, **kwargs):
...
def output_block(inputs, filters=0, activation="relu", num_classes=1000, drop_rate=0, classifier_activation="softmax",
                 is_torch_mode=True, act_first=False):
    nn = inputs
    if filters > 0:  # efficientnet like
        bn_eps = BATCH_NORM_EPSILON if is_torch_mode else TF_BATCH_NORM_EPSILON
        nn = conv2d_no_bias(nn, filters, 1, strides=1, use_bias=act_first, use_torch_padding=is_torch_mode,
                            name="features_")  # Also use_bias for act_first
    ...
    if num_classes > 0:
        ...
        nn = Conv2D(10, 1, activation='softmax')(UpSampling2D(size=(8, 8))(nn))
...
def global_context_module(inputs, use_attn=True, ratio=0.25, divisor=1, activation="relu", use_bias=True, name=None):
    ...
    hidden_activation, output_activation = activation if isinstance(activation, (list, tuple)) else (
    activation, "sigmoid")
    ...
    if use_attn:
        ...
        attn = functional.reshape(attn, [-1, 1,
                                         height * width])  # [batch, height, width, 1] or [batch, 1, height, width] -> [batch, 1, height * width]
        ...
        context = functional.reshape(context, [-1, 1, 1, filters]) if is_channels_last else functional.reshape(context,
                                                                                                               [-1,
                                                                                                                filters,
                                                                                                                1, 1])
    else:
...
def se_module(inputs, se_ratio=0.25, divisor=8, limit_round_down=0.9, activation="relu", use_bias=True, use_conv=True,
              name=None):
    ...
    se = se if use_conv else functional.reshape(se,
                                                [-1, 1, 1, filters] if image_data_format() == "channels_last" else [-1,
                                                                                                                    filters,
                                                                                                                    1,
                                                                                                                    1])
...
def eca_module(inputs, gamma=2.0, beta=1.0, name=None, **kwargs):
    ...
    nn = layers.Conv1D(1, kernel_size=kernel_size, strides=1, padding="valid", use_bias=False,
                       name=name and name + "conv1d")(nn)
...
def addaptive_pooling_2d(inputs, output_size, reduce="mean", data_format="auto", name=None):
    ...
    return reduce_function(pool_size=(h_pool_size, w_pool_size), strides=(h_strides, w_strides),
                           name=name and name + "pool")(inputs)
...
def __anti_alias_downsample_initializer__(weight_shape, dtype="float32"):
    ...
    kernel_size, channel = (weight_shape[0], weight_shape[2]) if backend.image_data_format() == "channels_last" else (
    weight_shape[2], weight_shape[0])
...
def fold_by_conv2d_transpose(patches, output_shape=None, kernel_size=3, strides=2, dilation_rate=1, padding="same",
                             compressed="auto", name=None):
...
class CompatibleExtractPatches(layers.Layer):
    def __init__(self, sizes=3, strides=2, rates=1, padding="same", compressed=True, force_conv=False, **kwargs):
        ...
        self.dilation_rate = (rates if len(rates) == 2 else rates[1:3]) if isinstance(rates, (list, tuple)) else (
        rates, rates)
    ...
    def call(self, inputs):
        ...
        if self.use_conv:
            ...
            if self.compressed:
                ...
                out = functional.reshape(out,
                                         [-1, conv_rr.shape[1], conv_rr.shape[2], self.kernel_size, self.kernel_size,
                                          self.channel])
        else:
            out = functional.extract_patches(inputs, self._sizes_, self._strides_, self._rates_,
                                             "VALID")  # must be upper word VALID/SAME
            if not self.compressed:
                # [batch, hh, ww, kernel, kernel, channnel]
                out = functional.reshape(out, [-1, out.shape[1], out.shape[2], self.kernel_size, self.kernel_size,
                                               self.channel])
        return out
...
class PreprocessInput:
    ...
    def set_input_shape(self, input_shape):
        ...
        if None in input_shape:
            ...
            channel_axis, channel_dim = min(enumerate(input_shape),
                                            key=lambda xx: xx[1])  # Assume the smallest value is the channel dimension
