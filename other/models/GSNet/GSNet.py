...
class GSNet(nn.Module):
    ...
    def __init__(self, input_dim, output_dim,
        ...
        self.start_out = int((self.input_dim[1] - np.sum([x-1 for x in kernel_sizes]) - self.output_dim[1]) / 2)
        self.a = nn.Upsample(scale_factor=1.49, mode='bilinear', align_corners=True)
...
if __name__ == '__main__':
    ...
    layers = [128, 64, 64, 32, 32, 32, 10]
    strides = [9] * 2 + [7] * 3 + [5] * 2
    model = GSNet([4, 128, 128], (128,128), layers, strides)
    out = model(x)
    print(out.shape)