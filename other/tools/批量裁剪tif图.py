...
def cut_image_overlap(image, block_size, overlap_rate, save_path, start_block_num):
    ...
    dep = image.shape[2] if len(image.shape) == 3 else 1  # 支持灰度图像
    ...
    # 创建一个用于填充的图像
    padding_img = np.zeros((padding_h, padding_w, dep), dtype=image.dtype)
...
if __name__ == "__main__":
    input_folder = r"C:\Users\zzc\Desktop\other\datasets\jiaozhouwan\256\img"  # 输入文件夹路径
    output_folder = r"C:\Users\zzc\Desktop\other\datasets\jiaozhouwan\128\img"  # 输出文件夹路径
    block_size = 128  # 块大小
    overlap_rate = 0  # 重叠率
