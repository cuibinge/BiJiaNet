...
def train(cfg):
    ...
    model = BuildingDetector(cfg)  # 传递 cfg 对象
    model = model.to(device)

    input_size = (3, cfg.DATASETS.ORIGIN.HEIGHT, cfg.DATASETS.ORIGIN.WIDTH)
    ...
    for epoch in range(start_epoch + 1, arguments['max_epoch'] + 1):
        ...
        for it, (images, annotations) in enumerate(train_dataset):
            logger.info(f"Iteration {it} - Images shape: {images.shape}")
            logger.info(f"Iteration {it} - Annotations: {annotations}")
    ...
    logger.info(
        "Total training time: {} ({:.4f} s / epoch)".format(
            total_time_str, total_training_time / (arguments['max_epoch'])
        )
    )
...
if __name__ == "__main__":
    ...
    # 检查数据集路径
    logger.info(f"Checking dataset path: {cfg.DATASETS.TRAIN}")
    if not os.path.exists(cfg.DATASETS.TRAIN):
        logger.error(f"Dataset path {cfg.DATASETS.TRAIN} does not exist.")
        exit(1)
